...
def dashboard():
    ...
    rooms=[get_room(room['_id']['room_id']) for room in rooms]
...
def chat_page(room_id):
    ...
    if is_room_member(room_id,current_user.username):
        ...
        return render_template('chat_page.html',username=current_user.username,room=room,messages=messages,current_user=current_user)
    else:
        return redirect(url_for('dashboard'))
###############################################################################################
...
def grp_page(room_id):
    ...
    if request.method=='POST':
        ...
        if "name" in request.form:
            ...
            rooms_collection.update_one({'_id':ObjectId(room_id)},{'$set':{'room name':name}})
            flash('Group info updated')


        if  request.files:
            print("\n\n yo \n\n")
            imageDict=upload(request.files['pic'])
            print("\n\n",imageDict,"\n\n")
            rooms_collection.update_one({'_id':ObjectId(room_id)},{'$set':{'profile_pic':imageDict['url']}})
...
def join_groups():
    ...
    if request.method=='POST':
        ...
        if not grp_info:
            flash("please fill the required field")
...
@app.route('/host_event/<room_id>')
@login_required
def host_event(room_id):
...
def message(data):
    ...
    image=get_user(data['username'])['image_file']

    save_message(data['room'],data['msg'],data['username'],image)
    
    send({'msg':data['msg'],'username':data['username'],'time_stamp':strftime('%b-%d %I:%M%p',localtime()),'pic':image},room=data['room'])
